#!/usr/bin/env python3
"""Process Audit Work March 50.xlsx with corrected WO logic - exactly 6 working days between WOs."""
...
# Helper to check if record should be skipped
...
for emp_id in df['Empl Id'].unique():
    ...
    # Process: count exactly 6 non-skipped days, then assign WO
    current_week = 1
    ...
    for i in range(first_wo_idx + 1, len(emp_indices)):
        ...
        is_skipped = should_skip(shift, remarks)
        
        days_counter += 1
        
        if is_skipped:
            # Preserve original, don't count as working day
        ...
        if working_days_in_week >= 6 or is_source_wo:
            # Assign most common shift to previous week
            ...
            if pd.notna(shift) and str(shift) not in ['0', 'WO', 'nan']:
