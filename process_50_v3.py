#!/usr/bin/env python3
"""Process with random shift selection from Sheet2 when shift changes."""
...
# Extract ALL shift timing rows from Sheet2 (not just unique shifts)
shift_timing_rows = []
...
print(f"Total shift timing rows collected: {len(shift_timing_rows)}")
...
for emp_id in df['Empl Id'].unique():
    ...
    # Process: count exactly 6 days, then assign WO
    current_week = 1
    days_in_week = 0
    week_shifts = []
    week_indices = []
...
for idx, row in df.iterrows():
    ...
    # Skip if Week_Shift is WO or empty
